...
int main(int argc, char *argv[])
{
    ...
    char junk[MAXLINE];
    int command_number, command_number2;   // number found with command
    int num_items;
...
/*
    ...
    printf("Queue      : ADDREAR; APPENDREAR RMFRONT; PRINTQ\n");
    printf("           : STATS; QUIT; SORTGEN; SORTIP\n");
*/
    // this list is sorted 
...
//        num_items = sscanf(line, "%s%d%s", command, &command_number, junk);
        ...
        if (num_items == 1 && strcmp(command, "ALERT") == 0) {
            ...
            ids_sort_list(unsorted_list, command_number, 1); 
        } else if (num_items == 2 && strcmp(command, "SORTIP") == 0) {
            ids_sort_list(unsorted_list, command_number, 2); 
        } else if (num_items == 3 && strcmp(command, "APPENDREAR") == 0) {
            ids_append_rear(unsorted_list, command_number, command_number2);

 ...
 * unsorted, and it does not have any limits on the maximum size of the list.
 *
 * This doesn't check is the new alert has the same destination IP address 
 * as an alert that is already in the list.
 */
void ids_append_rear(llist_t *list_ptr, int newgenid, int newdestip)
{
...
//    ids_record_fill(new_ptr);
    ...
    //Gen id and dest ip are sent over as well. 
    new_ptr->generator_id=newgenid;
    new_ptr->dest_ip_addr=newdestip;

    /* Set all other parts of element to 0 */
    new_ptr->signature_id=0;
    new_ptr->revision_id=0;
    new_ptr->src_ip_addr=0;
    new_ptr->dest_port_num=0;
    new_ptr->src_port_num=0;
    new_ptr->timestamp=0;
    ...
    llist_insert(list_ptr, new_ptr, LLPOSITION_BACK);
//    printf("Appended %d,%d onto queue\n", new_ptr->generator_id, new_ptr->dest_ip_addr);
}
...
void ids_sort_list(llist_t *L, int sort_type, int sort_by)
{
    clock_t start, end;
    double elapse_time;  // time in milliseconds

    int initialcount= llist_entries(L);

    start = clock();

    /* Sorted by genid */
    if(sort_by == 1)
        llist_sort(L, sort_type, ids_compare_genid);

    /* Sorted by ip */
    else
        llist_sort(L, sort_type, ids_match_destip);
    ...
    assert(llist_entries(L) == initialcount);
